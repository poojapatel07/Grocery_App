...
public class RvTodaySpecialListADP extends RecyclerView.Adapter<RvTodaySpecialListADP.MyViewHolder> {
    ...
    public interface PassValue_ProductDeatlis{
        void passvalueProductDetail(int pos, String product_image, int product_id, String product_name,
                                    int product_mrp, float product_retail, String margin, float TotalPrice,
                                    int cartQuanty, int In_Cart_qunty, int min_order_qunaty, int IS_FAV);
    }
...
//                                Integer.valueOf(binding.tvProductMOQ.getText().toString()),
...
//                            Utility.BADGE_PRICE);
    ...
    public void onBindViewHolder(@NonNull RvTodaySpecialListADP.MyViewHolder holder, int position) {
        ...
        if (allProductList.get(position).getIs_free_product().equals("1")) {
            holder.binding.LLFreeProduct.setVisibility(View.VISIBLE);
        } else {
            holder.binding.LLFreeProduct.setVisibility(View.GONE);
        }
